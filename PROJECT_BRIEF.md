...
目前進度:**Phase 1~6 已完成**;**Phase 7 已定規格,待實作**。
...
4. **赤壁內容**:已完成。完整 events.json 分鏡、火攻粒子、箭雨/爆炸/ring fx、旁白 quote 字卡、大氣系統。
5. **打磨**:已完成。選單卡片視覺、結算畫面、war-meter、電影感 overlay、四語補齊、章節 BGM、錄影模式。
6. **美術升級(6-A ~ 6-F)**:已完成。全域調色+光照、漸層天穹+FogExp2、水面 shader、夜戰點光源、地形 fbm 細節、旗幟飄動+風力。詳見《規格書.md》第十節。

### Phase 7 ─ 赤壁事件擴充(待實作)

> 緣起:第一章鋪陳段常有「發呆」感(字卡之間只有無利害的小幅巡江);且《三國演義》赤壁前哨多個關鍵軍事事件未演出或只用字卡帶過。
> 目標:依《三國演義》第四十三〜五十回補齊有戲劇張力的事件與軍事行動,讓畫面全程有事發生、因果鏈完整。
> 完整規格與逐單位移動座標見《Phase7規格.md》。

新增兩項引擎能力 + 四隊新單位,內容分六個子階段:

- **7-A 引擎地基**:中央事件 cut-in(橫排大字,事件觸發時淡入/停留/淡出,純 p 函數確定性)＋ `status.label` 指揮官改名。
- **7-B 曹操三笑**:尾聲華容道「三道伏兵」改寫為《演義》名場面三笑三拍(每笑一聲,趙雲/張飛/關羽依序殺出)。
- **7-C 三江口 + 反間計換帥**:新增「蔡瑁・張允水軍」,演出第四十五回三江口之戰;反間計後水軍都督伏誅、當場改名「毛玠・于禁水軍」(以旱將代水帥,埋連環計伏筆)。
- **7-D 新單位 + 闞澤龐統**:闞澤夜渡下詐降書、龐統獻連環計(小舟渡江,水軍收攏成連環陣)。
- **7-E 火攻加料**:黃蓋中箭落水→韓當救起、甘寧斬蔡中、諸路登陸夾擊烏林、張遼護主斷後。
- **7-F 重算 + 驗收**:兵力 losses/attrition 重算、回填驗收斷言、三章整跑。

兵力守恆:開場北軍 200,000 / 南軍 50,000 不變,新單位兵力一律從同側既有單位扣出。
