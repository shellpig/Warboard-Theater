...
1. **骨架**:選單頁 + 引擎頁路由、地形生成(含水面)、單位放置與名牌
2. **時間軸**:events.json 驅動單位移動/交戰狀態、播放控制
3. **導播運鏡**:camera state machine、事件鏡頭、手動接管邏輯
4. **赤壁內容**:完整 events.json 分鏡、火攻粒子特效、旁白字卡
5. **打磨**:選單卡片視覺、結算畫面、音效、錄影模式

每階段以 local server 在瀏覽器驗收後再進下一階段。
**請先輸出架構規劃(檔案結構 + schema 定版 + 階段拆分)經確認後再實作。**
